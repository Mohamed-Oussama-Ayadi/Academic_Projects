    <?php  
include_once '../../controller/facturesController.php';
include_once '../../model/factures.php';
include_once '../../controller/error.php';

$fac=new facturesC();

$listeFacture=$fac->afficherFactures();
$listeFactureRefernce=$fac->afficherRefernce();
$listeFactureStation=$fac->afficherIdStation();
...
)
{
 if(!empty($_POST['id_station']) &&
  ...
  !empty($_POST['montant']) && 
  !empty($_POST['datepaiement'])
 )
 {
  if($facC->verifiermatricule($_POST['matricule']) && $facC->verifierMontant($_POST['montant']))
  {
  $fac = new factures($_POST['reference'],$_POST['id_station'],$_POST['id_user'],$_POST['matricule'],$_POST['montant'],$_POST['datepaiement']);
  $facC->ajouterFactures($fac);
...
<section class="content-main" style="max-width: 720px">
	...
	<div class="content-header">
		<h2 class="content-title">Ajouter Factures</h2>
	</div>
<div id="error">
<?php echo $error ?>
</div>
  ...
  <form action ="" method="POST">
	<div class="card mb-4">
          <div class="card-body">
				<div class="mb-4">
					<label for="id_station" class="form-label">Identifiant Station</label>
<select class="form-select" name="id_station" id="id_station">
<?php
foreach($listeFactureStation as $facturel)
{
?>
 <option selected="Identifiant Station"><?php echo $facturel['id_station']; ?></option>
<?php
}
?>
</select>
				...
				<div class="mb-4">
					<label for="id_user" class="form-label">Identifiant Utilisateur</label>
					<input type="text" placeholder="Type here" class="form-control" name="id_user" id="id_user">
				</div>
				<div class="mb-4">
					<label for="matricule" class="form-label">Matricule</label>
					<input type="text" placeholder="Type here" class="form-control" name="matricule" id="matricule">
				</div>
				<div class="mb-4">
					<label class="form-label">Montant</label>
					<input placeholder="Type" type="text" class="form-control" name="montant" id="montant">
				</div>

				<div class="mb-4">
					<label for="datepaiement" class="form-label">Date Paiement</label>
					<input type="date" placeholder="Type here" class="form-control" name="datepaiement" id="datepaiement">
				</div>
...
<input type="submit"  class="btn btn-primary" id='boutton_ajouter' name='boutton_ajouter' value="Ajouter"/>
          </div>
    </div> <!-- card end// -->
</form>

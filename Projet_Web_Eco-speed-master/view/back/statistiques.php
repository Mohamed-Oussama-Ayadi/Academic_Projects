...
$query=$con->query("SELECT ville as ville_amount , sum(nombre_voiture) as amount from station group by ville_amount");
...
$query2=$con->query("SELECT ville as ville_amount , sum(energie) as energie_somme from station group by ville_amount");
foreach($query2 as $data2)
{
 $villetab2[] = $data2['ville_amount'];
 $col2[]=$data2['energie_somme'];
}
...
<script type="text/javascript">
  const labels2 = <?php echo json_encode($villetab2) ?>;
  const data2 = {
    labels: labels2,
    datasets: [{
      label: 'Energie Consommee par Ville',
      data2: <?php echo json_encode($col2) ?>,
      backgroundColor: [
        'rgba(255, 99, 132, 0.2)',
        'rgba(255, 159, 64, 0.2)',
        'rgba(255, 205, 86, 0.2)',
